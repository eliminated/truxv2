...
- Direct messages with inbox/search, live thread polling, read receipts, attachments, edit/unsend windows, and profile entry points
- Post reporting with optional private DM updates from Report System Updates
- Staff-only moderation area with report queues, suspicious activity review, audit logs, review workspaces, and admin-only staff access management
...
- Account/settings tools for notification preferences, muted users, email verification, linked accounts, and password changes
...
- Command-shell UI overhaul across app, auth, messages, settings, profile, and moderation surfaces
...
- `database/migrations/20260323_complete_moderation_v050.sql`
...
- `database/migrations/20260330_reset_all_email_verification.sql`
- `database/migrations/20260330_upgrade_linked_accounts_v061.sql`
- `database/migrations/20260330_upgrade_direct_messages_v2.sql`
...
For legacy DM attachment cleanup audits, run:

```bash
php scripts/audit_dm_attachment_orphans.php
```

## UI Baseline
- The site now runs on the command-shell redesign introduced in the v0.5.x overhaul.
- The old visual toggles and stored UI-preference columns were removed during the redesign cleanup.
- Message, moderation, and mobile surfaces now use the same redesign system rather than the earlier classic layout.
...
- Direct messages for 1-to-1 conversations with live polling, unread sync, read receipts, attachment delivery, edit/unsend windows, and inline moderation reporting
- Post reporting with optional report-update DMs from Report System Updates
- Staff-only moderation tools with role-based access, review queues, suspicious activity review, audit logs, archived reports, and an admin-only staff access page with confirmed role changes
...
- Account settings for notifications, muted users, email verification state, password changes, and linked-account management
...
- Email verification and recognized-domain trust hints
- Linked-account provider scaffolding for Discord, Google, Facebook, X, and Nicholic Account
